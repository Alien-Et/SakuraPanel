...
<body>
  ...
  <div class="content">
    <h1>💔 哎呀，D1没绑定哦</h1>
    <p>喂！大臭宝，你的 <span class="highlight">Cloudflare D1 数据库</span> 还没绑定呢~<br>快去 <span class="highlight">Cloudflare Workers</span> 设置里绑一个 D1 数据库（变量名：<span class="highlight">D1DB</span>），然后重新部署一下吧！</p>
...
export function 生成错误页面(错误信息) {
  ...
  const 是D1错误 = msg.includes('d1') || msg.includes('429') || msg.includes('quota') || msg.includes('exceeded') || msg.includes('rate limit');
  // 转义 HTML 特殊字符，防止错误信息中的 <>&" 破坏页面结构
  const 转义信息 = (错误信息 || '未知错误').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');

  const 标题 = 是D1错误 ? 'D1配额已用完' : '服务器开小差了';
  const 说明 = 是D1错误
    ? 'Cloudflare D1 数据库的每日免费配额已经用完啦~<br>请等待 <span class="highlight">每天 UTC 0点</span> 自动重置，<br>或前往 Cloudflare 控制台查看配额使用情况。'
