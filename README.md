...
樱花面板是一个基于Cloudflare Workers的代理管理面板，提供简洁美观的用户界面，支持VLESS协议和灵活的节点管理。该面板使用中文界面，操作简单直观，适合各类用户使用。
...
- **节点管理**：支持手动上传节点（追加+去重）、添加/移除节点路径，每次订阅请求实时拉取并按"地址:端口"智能去重
- **代理配置**：支持VLESS协议，可生成Clash、通用客户端和SingBox配置
- **代理模式**：支持直连、反代、SOCKS5等多种连接方式
- **UUID管理**：支持更换UUID，方便用户管理订阅
- **Base64加密**：仅对通用订阅进行Base64加密，Clash和SingBox保持原生格式
- **订阅Token验证**：基于SHA-256生成订阅Token，防止订阅链接被滥用
- **CF请求统计**：通过Cloudflare GraphQL API自动查询Workers/Pages当日请求数，映射到Subscription-Userinfo响应头，客户端可显示请求用量
- **ECH加密**：支持ECH（加密客户端问候），提高连接稳定性
- **花瓣飘落效果**：全界面樱花花瓣飘落动画，支持横向摇摆
- **响应式设计**：适配各种设备屏幕，支持明暗主题切换
- **隐藏滚动条**：优选IP列表区域保持可滚动但无视觉滚动条，界面更整洁
...
点击 README 顶部的 **Deploy to Cloudflare** 按钮，授权后自动关联本仓库并部署。
...
- **功能**：订阅面板主界面
- **行为**：
  - 显示用户订阅面板，包含UUID、代理设置、加密设置、ECH设置、机场设置、壁纸设置、优选IP网络路径、上传优选节点、订阅链接等
  - 需要登录状态才能访问
...
- **功能**：WebSocket连接路径，用于VLESS协议传输
- **行为**：
  - 处理WebSocket升级请求
  - 支持VLESS over WebSocket传输
  - 自动处理Early Data和连接代理逻辑
...
2. 点击"添加路径"按钮
3. 系统会先预拉取该URL进行验证，并反馈拉取状态（成功/失败）和节点数量
4. 每次订阅请求时**实时**从所有路径拉取最新节点并合并去重，不再使用缓存，添加/删除路径后立即生效
...
2. 启用后，仅对通用订阅（V2Ray/V2RayNG/V2RayN）进行Base64加密
3. Clash（猫咪）和SingBox订阅始终保持原生YAML/JSON格式，不进行加密
4. 点击开关后立即生效，无需额外操作

### 订阅Token验证

1. 在"订阅Token验证"部分，打开开关启用Token验证
2. Token基于当前域名和UUID自动生成（SHA-256），无需手动填写
3. 启用后，订阅链接自动附加 `?token=xxx` 参数
4. 更换UUID后Token自动更新，旧链接立即失效
5. 未带正确Token的订阅请求将被拒绝

### CF请求统计

1. 在"CF请求统计"部分，选择认证方式（二选一）：
   - **API Token**：填写Account ID和API Token（需Account Analytics: Read权限）
   - **邮箱+Global API Key**：填写CF登录邮箱和Global API Key
3. 点击"刷新用量"查看当日Pages和Workers请求量
4. 启用后，订阅响应头自动注入 `Subscription-Userinfo`，客户端可显示请求用量
5. 查询结果缓存5分钟，避免频繁调用API
6. 凭证保存后脱敏显示，点击输入框可查看真实值编辑

### 订阅配置

#### 获取订阅链接

1. 在主面板中，找到"Clash订阅"、"通用订阅"或"SingBox订阅"部分
2. 复制显示的订阅链接
3. 在支持的客户端中导入该链接
...
1. **完整格式**：`[地址]:端口#节点名称@tls/notls`
   ...
   - 示例：`[2001:db8::1]:80#日本-01@notls`

2. **不带端口格式**：`[地址]#节点名称@tls/notls`
   - 示例：`1.2.3.4#美国-01@tls`
   - 系统将使用默认端口（`tls` 为 443，`notls` 为 80）
...
4. **最简格式**：`[地址]`
   - 示例：`1.2.3.4`
   - 系统将使用默认端口（`tls` 为 443，`notls` 为 80）、默认协议 `tls`、默认节点名称"🌸樱花"

#### 格式说明

- **地址**：支持IPv4地址（如`1.2.3.4`）、IPv6地址（如`[2001:db8::1]`）和域名（如`example.com`或`sub.example.com`）
- **端口**：范围1-65535，未指定时使用默认值（`tls` 为 443，`notls` 为 80）
...
系统对来自不同来源（URL路径拉取、手动上传）的节点统一按 **`地址:端口`** 去重，避免同一节点因格式差异而重复出现：

- **统一解析**：新增 `解析节点()` 函数兼容所有格式（`[IPv6]:端口#名@tls`、`IPv4:端口#名@tls`、`域名#名`、纯地址等），提取 `地址小写:端口` 作为去重键（IPv6去括号、域名/IP统一小写、端口缺省：`tls`=443，`notls`=80）
- **优先级**：手动上传节点优先于URL拉取节点（手动节点已标准化且带自定义名称，会覆盖同IP:端口的URL节点）
- **上传合并**：手动上传时，新节点追加到现有列表并按去重键合并，重新上传同IP:端点的节点会覆盖旧条目以更新名称
- **示例**：URL里的 `1.2.3.4` 与手动上传的 `1.2.3.4:443#日本@tls` 会被识别为同一节点，最终保留手动上传的那条
...
- **WebSocket路径**：默认使用`/?ed=2560`作为WebSocket路径，支持Early Data
...
系统会在控制台输出详细的连接日志：
- `[智能连接] 强制反代: 候选=...` - 强制反代模式
- `[智能连接] 强制SOCKS5: ...` - 强制SOCKS5模式
- `[智能连接] 直连失败，动态切换代理: ...` - 动态模式切换
- `[智能连接] 动态反代: 候选=...` - 动态反代切换
- `[智能连接] 动态SOCKS5: ...` - 动态SOCKS5切换
- `[直连] 开始/成功/失败: ...` - 直连模式

### 错误处理

- 系统会记录连接失败日志
- 单条节点路径拉取失败时跳过该路径，不影响其他路径的节点加载
- 所有节点来源（URL路径+手动上传）均失败时，使用部署域名 `hostName:443` 兜底
- 订阅配置每次请求实时生成，无缓存层，确保与最新节点列表同步
...
### Q: 启用Base64加密后，哪些订阅会受影响？

A: Base64加密仅对**通用订阅**（V2Ray/V2RayNG/V2RayN）生效。Clash（猫咪）订阅始终返回YAML格式，SingBox订阅始终返回JSON格式，不受Base64加密开关影响。这是为了保证各客户端的兼容性。
...
**⚠️ 强烈建议使用自定义域名**：
- 部分地区运营商会针对`*.workers.dev`或`*.pages.dev`默认域名进行封锁
- 使用自定义域名可以显著降低被封锁的风险
- 即使在Clash系列客户端显示timeout时，使用自定义域名仍有80%几率连接成功
- 自定义域名设置方法详见[自定义域名设置](#自定义域名设置)部分
...
**💡 重要提示**：如果您使用的是默认的`*.workers.dev`或`*.pages.dev`域名，强烈建议设置自定义域名：
- 默认域名容易被运营商针对性封锁
...
### v1.3.0
- 新增订阅Token验证功能：
  - 基于SHA-256(host+UUID)生成Token，订阅请求需带`?token=`参数
  - 更换UUID后Token自动更新，旧链接失效
- 新增CF请求统计功能：
  - 通过Cloudflare GraphQL API查询当日Workers/Pages请求数
  - 支持API Token和邮箱+Global API Key两种认证方式（二选一）
  - 映射到Subscription-Userinfo响应头，客户端可显示请求用量
  - 查询结果5分钟缓存，凭证脱敏显示
- 修复Base64加密逻辑：
  - Clash和SingBox订阅不再进行Base64加密，保持原生YAML/JSON格式
  - 仅通用订阅（V2Ray）保留Base64加密
- 新增全界面花瓣飘落动画效果（共享模块，统一管理）
- 所有界面底部添加项目驱动署名
- 代码重构：提取花瓣效果为共享模块，减少冗余

### v1.2.4
- 新增SingBox配置和订阅支持：
  - 新增SingBox配置生成，输出标准JSON格式配置文件
  - 新增SingBox订阅链接，支持直接导入SingBox客户端
  - 订阅面板标题改为"📦 SB订阅"
- 优化优选IP列表显示：
  - 移除列表区域滚动条视觉样式，保持可滚动但界面更整洁
- 优化代码隐蔽性：
  - 前端标题、注释、错误日志中的SingBox相关明文改为atob()形式
- 修复若干UI细节问题

### v1.2.3
- 优化配置文件下载文件名格式：
  - 猫咪配置（Clash）自动添加`.yaml`后缀，便于客户端识别
  - 通用配置（V2Ray/V2RayNG/V2RayN）自动添加`.txt`后缀，符合标准格式
