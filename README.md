...
2. **手动解密配置**：
   - 下载Base64格式的配置文件（文件名通常为`clash-config.b64`）
...
- 修复代理地址配置问题：
  ...
  - 完善代理模式文档说明
  - 优化代理连接逻辑和错误处理
  - 加密模式的clash配置，经测试flclash、clash Meta for Android和nekobox暂时无法解密(我也不知道配置文件带了什么解密原理啊)，v2rayn客户端通用配置可以自动解密
  - 不要在意clash客户端显示timeout，使用flclash能显示落地ip就是能连(随意点击ipv4/ipv6节点连接)，nekobox、v2rayng除外。
  - 在面板打开强制代理后，flclash会显示反代/sk5的落地ip和归属地，无论你选择的是哪个归属地的优选ip都会强制显示为反代/sk5的落地ip和归属地
