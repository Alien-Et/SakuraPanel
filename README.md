...
- **统一解析**：新增 `解析节点()` 函数兼容所有格式（`[IPv6]:端口#名@tls`、`IPv4:端口#名@tls`、`域名#名`、纯地址等），提取 `地址小写:端口` 作为去重键（IPv6去括号、域名/IP统一小写、端口缺省443）
...
- 手动上传节点由覆盖改为**追加+去重**，新节点追加到现有列表并按去重键合并，重新上传同IP:端口节点可更新名称
