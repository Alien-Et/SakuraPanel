# 樱花面板增强版 - 详细使用说明
### ⚠️注意！！
1.使用中国移动网络，包你timeout！因为pages.dev已经被移动屏蔽，workers域名你们自行测试(我没测试)。(国内部分地市套自定义域名可以使用，也有些地市套自定义域名也一样timeout)，总之移动不行！！
...
### 🌸樱花面板主界面一览
只要把KV空间设置好了后，基本上所有操作都能在webui前端操作，不需要到cloudflare后台设置环境变量。
